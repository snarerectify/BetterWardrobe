...
namespace snare\BetterWardrobe\utils;
...
class Utils
{
    ...
    {
        if (!isset($data["item"]) || ($data["item"] === "air")) return VanillaItems::AIR();

        try {
            $item = StringToItemParser::getInstance ()->parse($data["item"]);

            if($item === null) return VanillaItems::AIR();

            if (isset($data["count"])) {
                if (is_string($data["count"])) {
                    $item->setCount(eval($data["count"]));
                } elseif (is_int($data["count"])) {
                    $item->setCount($data["count"]);
                }

                if ($item->getCount() === 0) return VanillaItems::AIR();
            }

            if ($item instanceof Tool && !$item->isUnbreakable()) $item->setDamage($data["damage"] ?? 0);

            if ($item instanceof Armor) {
                $rgb = $data["color"] ?? null;

                if ($rgb !== null) $item->setCustomColor(new Color($rgb[0] , $rgb[1] , $rgb[2] , $rgb[3] ?? 255));
            }

            if (isset($data["customName"])) $item->setCustomName(TextFormat::colorize($data["customName"]));
            ...
            if (isset($data["enchantments"])) {
                foreach ($data["enchantments"] as $str) {
                    ...
                    try {
                        ...
                        if ($enchantment !== null) $item->addEnchantment(new EnchantmentInstance($enchantment, (int)$en[1]));
                    } catch (Exception) {
